          ...
          {CONDITIONS.map(c=>(
            <div key={c.key} style={{marginBottom:6}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:"0 6px"}}>
                <FField sm label="Méd. 5A." value={data[c.key]?.media5||""} onChange={v=>onUpdate(c.key,"media5",v)} />
...
// PNG download — toSvg + canvas manual (compatível com CSS Grid e ambiente Vercel)
async function downloadCardPNG(elementId, filename) {
  const el = document.getElementById(elementId);
  if (!el) { alert('Elemento não encontrado: ' + elementId); return; }

  const domtoimage = window.domtoimage || __dtim__;
  if (!domtoimage) { alert('Biblioteca de imagem indisponível'); return; }

  // Inline imgs como base64 para evitar cross-origin taint (logos)
  ...
  try {
    // toSvg gera o SVG; desenhamos manualmente no canvas do browser (toBlob nativo é confiável)
